...
To be able to check out and update the submodules in this repository, you'll need to connect to GitHub via SSH. This is configured/determined by the contents of `.gitmodules` where the URLs for the different submodules are set to be `url = git@github.com:IFRCGo/go-api.git`. GitHub URLs that start with `git@` clone and pull over SSH.
...
3. Check out all submodules: `git submodule update --init --recursive`
