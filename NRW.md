...
2. Generate TypeScript types for talking to the various backends:
  ...
  2. In `app/` run `pnpm initialize:type`
  3. In the root of the repository run `pnpm generate:type`, this should exit successfully.
3. Copy [`sample.env`](./app/sample.env), rename it to `.env` and set any needed values there.
4. **NOTE**: You can set `APP_NRW_STANDALONE` to `false` or `true` for respectively *embedded* or *standalone* version. If the former, then set up and run the `go-api` submodule service first. See [go-api README](./go-api/README.md).
5. The FontAwesome API token must be available when installing dependencies: `FONTAWESOME_API_KEY=XXXX pnpm install`.
6. Launch with `pnpm start` and visit `http://localhost:3000`.
7. In embedded version: go to Country > Country Profile > National Risk Watch. Standalone version is directly accessible at `http://localhost:3000`.
...
2. From `/app/`, run `pnpm generate:type:nrw` from the repository root
