...
- Map URL search params: this prototype branch uses a `useNrwMapSearchParams` hook (`app/src/utils/nrw/hooks/useNrwMapSearchParams.ts`). The new main branch has no `app/src/utils/nrw/` directory; instead a view-local `useNrwSearchParams` hook (`app/src/views/CountryProfileNationalRiskWatch/hooks/useNrwSearchParams.ts`) owns the URL search params (`z`, `lat`, `lon`, `countries`), scoped-countries resolution (standalone mode: from URL; embedded mode: from the routed country via `useCountry`), and the `handleMapViewChange` writer. The parse/serialize helpers (`parseZoomUrlParameter`, `parseMapLatitudeParameter`, `parseMapLongitudeParameter`, `parseCountriesUrlParameter`, `serializeCountriesUrlParameter`, `parseCountryCode`) live inside that same hook file — there is no `hooks/utils.ts`. The hook returns `zoomFromUrlParams`, `latitudeFromUrlParams`, `longitudeFromUrlParams`, `countries`, `countriesResolved`, `handleMapViewChange`. Opaque `Zoom`/`Latitude`/`Longitude`/`CountryCodeIso3` types plus `InitialMapView` (`zoom`, `center`, optional `fitBounds`) and `LongitudeLatitudeBounds` live in the view-level `types.ts` and `NrwLngLat.ts`. The view-level `utils.ts` only holds admin-area query/bounds helpers (`getAdminArea0Query`, `getFeatureCollectionBounds`).
- Initial map view resolution: the new main branch view (`app/src/views/CountryProfileNationalRiskWatch/index.tsx`) resolves the initial map view itself — precedence is URL lat/lon > fit scoped-countries bbox (fetched via `useNrwRequest` to `/admin-areas?filter=countryCodeIso3='XXX' AND adminLevel=0`) > defaults (zoom 3, center 0,0) — and gates `<NrwMap>` render on that resolution. The result is an `InitialMapView`; `<NrwMap>` takes `initialMapView` and `onMapViewChange` props (child components never touch URLs).
- NRW REST requests: in the new main branch, typed NRW request hooks (`useNrwRequest`, `useNrwLazyRequest`) live in `app/src/utils/restRequest/index.ts` (`apiType: 'nrw'`), using generated types in `app/generated/nrwTypes.ts`. The NRW API is now actively used: the view uses `useNrwRequest` for admin-area lookups, and `app/src/views/CountryProfileNationalRiskWatch/hooks/useNrwEvents.ts` fetches `/events` for a country via `useNrwRequest` — it is consumed by the `NrwMap` wrapper itself (currently with a hardcoded default country code `MWI`).
